...
getSearchBookData() {
  ...
  if (localStorage.getItem('token') === null) {
    this.matSnackBar.open('Veuillez vous connecter', 'ok', {
      duration: 5000
    });
  ...
  handleResponse(data: any) {
    console.log(data);
    this.matSnackBar.open('Plat ajouté!' , 'ok', {
    duration: 5000
  });
}
handleWishResponse(wishdata: any) {
  ...
  Deatails(bookId) {
    ...
    this.route.navigateByUrl('plats/info/' + bookId);
  ...
  getUpdatedNotes(event) {
    ...
    addtoWish( bookId: any) {
      if (localStorage.getItem('token') === null) {
        this.matSnackBar.open('Veuillez vous inscrire!', 'ok', {
          duration: 5000
        });
        this.route.navigateByUrl('login');
      }

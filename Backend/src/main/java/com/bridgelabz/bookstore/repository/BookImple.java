...
public interface BookImple extends JpaRepository<Book, Long> {
	...
	@Query( value = "select * from platinfo where  status='approved'", nativeQuery = true)
    List<Book> getApprovedBooks();
	
	@Query( value = "select * from platinfo where book_name like %?1% AND status='approved'", nativeQuery = true)
    Page<Book> findByBookName(String name, Pageable pageable);
	
	@Query( value = "select * from platinfo where status='OnHold'", nativeQuery = true)
	...
	@Query( value = "select * from platinfo where user_id=:id", nativeQuery = true)
	...
	@Query( value = "select * from platinfo where status='rejected'", nativeQuery = true)
